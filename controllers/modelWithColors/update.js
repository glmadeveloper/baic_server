...
const updateModelWithColors = async (req, res) => {
  try {
    const id = req.params.id;

    const test = await ModelWithColors.findOne({
      where: {
        id: id,
      },
    });
    ...
    if (test) {
      ...
      const model = await ModelWithColors.update(data, { where: { id: id } });
      return res.json({
        status: true,
        message: "Model With Colors Update Successfully .",
        data: model,
        code: 200,
      });
    } else {
      return res.json({
        status: false,
        message: "Model With Colors is Not Update",
        code: 404,
      });
    }
  } catch (error) {
...
export default updateModelWithColors;
